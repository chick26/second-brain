...
![1.1 Tokenization 分词-image.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782643861640_1.1_Tokenization_分词-image.png)
![1.1 Tokenization 分词-image-1.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782643854232_1.1_Tokenization_分词-image-1.png)
...
* 痛苦的真正根源是什么？因为分词。
![1.1 Tokenization 分词-image-7.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782643858136_1.1_Tokenization_分词-image-7.png)
...
![1.1 Tokenization 分词-image-6.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782643862833_1.1_Tokenization_分词-image-6.png)
![1.1 Tokenization 分词-image-11.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782643855202_1.1_Tokenization_分词-image-11.png)
...
1. **初始化大词表：** 左侧是corpus以及对应出现的次数，右侧是构建出的词表。这里的构建采用任何方式都行，如BPE，但要注意两点：所有的 **单个字符（如h,u,g）必须出现在词表当中**；**构建出的词表必须要能cover当前的corpus，不然下一步计算概率就会失败**
   ![1.1 Tokenization 分词-image-5.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782643859644_1.1_Tokenization_分词-image-5.png)
2. **挑选最大似然的分词结果：** 此处拿hug举例，hug在当前词表中可以暴力枚举有4种分词方法：`h,u,g`，`hu,g`，`h,ug`，`hug`，其中前3种都在词汇表中，第四种hug不在。可以计算出最大的那个概率1.11e-02
   ![1.1 Tokenization 分词-image-8.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782643861899_1.1_Tokenization_分词-image-8.png)
   对所有corpus都进行如下计算，之后求弄个加权求和，作为Loss，**初始的Loss总值为170.40**
   ![1.1 Tokenization 分词-image-2.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782643854800_1.1_Tokenization_分词-image-2.png)
3. **精简词汇表：** 从上图的结果可以看到，有一个token是没有使用的，就是 ug。精简词汇表的原则是移除影响loss最少的token。首先先从词汇表中删除 ug，可以看到即使删除 ug，根据上图的分词方法，Loss值不会有变化。理论上包括 ug 在内，剩下的 hu,pu,lu,bu,du 这几个词在第一轮精简的时候 **单独删除都不会影响Loss如下图左所示（例如：单独删除 hu，hug还可以表示为h, ug），** 所以我们可以随机删除一个token，就选择没有出现的 ug 吧
   ![1.1 Tokenization 分词-image-4.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782643861785_1.1_Tokenization_分词-image-4.png)
   ![1.1 Tokenization 分词-image-10.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782643858015_1.1_Tokenization_分词-image-10.png)
   token被删除后，会影响其他token的概率计算（因为基数从180个减少了，上图右），然后就是第二轮删除token。下图是删除各个单词后Loss的情况，bu对应的Loss最小，故删除bu。然后重复上述过程，直到达到设定的词表大小
   ![1.1 Tokenization 分词-image-3.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782643857736_1.1_Tokenization_分词-image-3.png)
...
![1.1 Tokenization 分词-image-9.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782643868468_1.1_Tokenization_分词-image-9.png)
