...
![](<../images/1.1 Tokenization 分词-image.png>)
![](<../images/1.1 Tokenization 分词-image-1.png>)
...
* 痛苦的真正根源是什么？因为分词。
![](<../images/1.1 Tokenization 分词-image-7.png>)
...
![](<../images/1.1 Tokenization 分词-image-6.png>)
![](<../images/1.1 Tokenization 分词-image-11.png>)
...
3. **精简词汇表：** 从上图的结果可以看到，有一个token是没有使用的，就是 ug。精简词汇表的原则是移除影响loss最少的token。首先先从词汇表中删除 ug，可以看到即使删除 ug，根据上图的分词方法，Loss值不会有变化。理论上包括 ug 在内，剩下的 hu,pu,lu,bu,du 这几个词在第一轮精简的时候 **单独删除都不会影响Loss如下图左所示（例如：单独删除 hu，hug还可以表示为h, ug），** 所以我们可以随机删除一个token，就选择没有出现的 ug 吧
   ![](<../images/1.1 Tokenization 分词-image-4.png>)
   ![](<../images/1.1 Tokenization 分词-image-10.png>)
   token被删除后，会影响其他token的概率计算（因为基数从180个减少了，上图右），然后就是第二轮删除token。下图是删除各个单词后Loss的情况，bu对应的Loss最小，故删除bu。然后重复上述过程，直到达到设定的词表大小
   ![](<../images/1.1 Tokenization 分词-image-3.png>)
...
![](<../images/1.1 Tokenization 分词-image-9.png>)
