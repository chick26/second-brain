...
## Cubox Review 增补：Chunk-then-Embed 的待验证线索

`先分块再向量化已经过时` 当前正文几乎全在图片中，未 OCR，不能直接沉淀具体方法论。先作为 RAG 优化方向的待验证线索：传统 chunk-then-embed 流程可能在复杂文档、表格、层级结构和多跳问题上丢失语义结构。

后续如果补证据，应优先核验：

- 是否提出了替代 chunk 策略，如层级索引、结构化解析、图索引或摘要索引。
- 是否区分了文档解析、切分、embedding、retrieval、rerank、generation 各阶段。
- 是否有真实评测指标，而不只是概念判断。

## 增补来源

- [[先分块再向量化已经过时-2025-12-13]]
