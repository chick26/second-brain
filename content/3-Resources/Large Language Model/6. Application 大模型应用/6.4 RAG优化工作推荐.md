...
![](<../images/6.4 RAG优化工作推荐-N9cxbIfe5ofY6JxMLv2cM95on1g.png>)
...
![](<../images/6.4 RAG优化工作推荐-FjInbLWwWouS9SxHkN2cKzDXnWb.png>)
...
![](<../images/6.4 RAG优化工作推荐-Mid2bOl90oj47dxLGKrcLQvcnHf.png>)
...
![](<../images/6.4 RAG优化工作推荐-SxaEbkiUAoMG35xvfXocUEN6nmf.png>)
...
![](<../images/6.4 RAG优化工作推荐-KCXDbonNxodNQwxN9RPcnsuVnGb.png>)
...
![](<../images/6.4 RAG优化工作推荐-JBBFbhsCToG2LyxW4EgcMPMnnUe.png>)
...
![](<../images/6.4 RAG优化工作推荐-LnEzbaxKvo8GULxN9R4csOqXnRd.png>)

* 模型效果
  * 不同模型触发(Trigger)自我矛盾数据对的比例，GPT4最低：
  ![](<../images/6.4 RAG优化工作推荐-UGVwbko0moGrQJxhPFCcs4venvb.png>)
  * ChatGPT作为aLM判别模型，在多种模型生成结果中的效果：
  ![](<../images/6.4 RAG优化工作推荐-AS6FbRRPQoqpqixiCwTcRR64n4f.png>)
  * gLM本身作为aLM判别模型效果：
  ![](<../images/6.4 RAG优化工作推荐-HrnmbWpr4otwoExeMF6cKbTFnGD.png>)
> **方案2**：SPO三元组引入

![](<../images/6.4 RAG优化工作推荐-ATXsb2QHkoukeZxXQw9cGEiSnKf.png>)
...
* 生成样式：SFT时候补充对应语料数据，规范格式化数据，提升用户体感
  * markdown格式&序号
  ![](<../images/6.4 RAG优化工作推荐-XFFsbx6C2obzqvxiIIrcij8FnWe.png>)
  * json格式
  ![](<../images/6.4 RAG优化工作推荐-Do1QbUXnHodIaaxODjLccLBHnwe.png>)
  * emoji表情
    ![](<../images/6.4 RAG优化工作推荐-D6D6bOsXNoOGIuxXj6rcPzjcnCe.png>)
    ![](<../images/6.4 RAG优化工作推荐-Adt1bzXLhozkDUxUQD2cLnRznwP.png>)
...
* [Lost in the Middle: How Language Models Use Long Contexts](https://arxiv.org/pdf/2307.03172.pdf)
  ...
  * ["大海捞针"实验启发](https://zhuanlan.zhihu.com/p/640641794)
  ...
  ![](<../images/6.4 RAG优化工作推荐-VIHNbDN1AoIEwZxYOdeck1jynDc.png>)
  ![](<../images/6.4 RAG优化工作推荐-CTW8b2egNoclgHxRSChcb0iDnJh.webp>)
* [RAPTOR: RECURSIVE ABSTRACTIVE PROCESSING FOR TREE-ORGANIZED RETRIEVAL](https://arxiv.org/pdf/2401.18059.pdf)
  ...
  * RAPTOR树查询，在不同抽象级别的文档中找出匹配片段。
    ...
    * 折叠树：折叠树就是全部平铺，用ANN库查询。
    ![](<../images/6.4 RAG优化工作推荐-LyuPbOYhNolLYnxQNGAcOkawnpg.png>)
  ...
  * [IN-CONTEXT PRETRAINING: LANGUAGE MODELING BEYOND DOCUMENT BOUNDARIES](https://arxiv.org/pdf/2310.10638.pdf)
    ...
    * 过程:
      ...
      * 基于改变顺序后的doc list进行pretrain训练
      ![](<../images/6.4 RAG优化工作推荐-R5qobqLbdowW21xjZ7FcDjRXn4c.png>)
      ![](<../images/6.4 RAG优化工作推荐-JRwAbRewgoRfGFxeH0zcofBpnmh.png>)
* **SFT侧:**
...
![](<../images/6.4 RAG优化工作推荐-AQ7obNFbZo7Hfpx5hZrcs3JTnUb.png>)

![](<../images/6.4 RAG优化工作推荐-NPzFbInBrobuy6xPYIDc8XA6njc.png>)
...
![](<../images/6.4 RAG优化工作推荐-WO7FbfsgtohkOixI0zgcgIZTnSd.png>)

![](<../images/6.4 RAG优化工作推荐-K2Y1bc36Fo7fQKxXVvzcdKMfnXe.png>)
...
![](<../images/6.4 RAG优化工作推荐-Mpghb2cSVofrUbxGwnocEHmAnpd.png>)
...
![](<../images/6.4 RAG优化工作推荐-QO2WbMJqBojAhdxFJQ7c36l7nWf.png>)

* 动态Premature层选择:

最优premature层应该是与最终层的输出差别最大，从而才能放大对比解码的有效性，采用下面的方式来度量两层在下一个单词预测分布的差异，JSD为Jensen-Shannon散度，premature层可以在给定候选层内选择

![](<../images/6.4 RAG优化工作推荐-LO9xbVW75o5tsWxQSUlcvZi8n9d.png>)

![](<../images/6.4 RAG优化工作推荐-AUYebpuoPoBcyvxwZZTc5NRWntb.png>)
...
![](<../images/6.4 RAG优化工作推荐-VYmNb34wXoRlWKxzDB9cofzDnaf.png>)

![](<../images/6.4 RAG优化工作推荐-ZBFfbV2Gyov49rxLQAwchJ4wnTd.png>)

![](<../images/6.4 RAG优化工作推荐-Rp6BbMsC1o6EIoxD1KucjCEJnqh.png>)

# 6.4.3 RAG增强

> 一句话概括：从预训练、有监督微调、推理等全阶段，都引入检索机制提升模型的效果。

![](<../images/6.4 RAG优化工作推荐-OzRubt2muoGWTmxVmt8c2Nzinjb.png>)
