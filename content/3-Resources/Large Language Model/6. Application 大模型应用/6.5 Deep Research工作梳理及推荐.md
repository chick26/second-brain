...
## Cubox Review 增补：IterResearch vs ReAct

`IterResearch和ReAct模式详细对比` 当前主要依赖图片，未 OCR。先保留一个方向判断：IterResearch 更像面向调研任务的“迭代研究 loop”，而 ReAct 是通用的 `Thought -> Action -> Observation` 工具循环。

后续补全时建议关注三点：

- 是否有显式研究计划和 gap 列表。
- 是否会根据中间证据改写下一轮查询。
- 是否有停止条件和证据质量评估，而不是无限搜索。

## 增补来源

- [[IterResearch和ReAct模式详细对比（附案例-2026-02-07]]
