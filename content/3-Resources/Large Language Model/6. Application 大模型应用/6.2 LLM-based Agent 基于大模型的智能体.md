...
![6.2 LLM-based Agent 基于大模型的智能体-image.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696151612_6.2_LLM-based_Agent_基于大模型的智能体-image.png)
...
![6.2 LLM-based Agent 基于大模型的智能体-diagram.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695798501_6.2_LLM-based_Agent_基于大模型的智能体-diagram.png)

## 智能体的历史沿革

从早年的规则驱动，到强化学习驱动，再到如今的 **大模型驱动（LLM-based Agents）**，智能体的能力在不断的提升，应用场景也大幅扩展

![6.2 LLM-based Agent 基于大模型的智能体-diagram-1.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695794052_6.2_LLM-based_Agent_基于大模型的智能体-diagram-1.png)
...
![6.2 LLM-based Agent 基于大模型的智能体-XOI9bpp4foLrajx2uYSc4YRjnng.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695780770_6.2_LLM-based_Agent_基于大模型的智能体-XOI9bpp4foLrajx2uYSc4YRjnng.png)

# 6.2.2 基于大模型的智能体的原理

LLM Agent是一种超越简单文本生成的人工智能系统。它使用LLM作为其核心计算引擎，使其能够进行对话、执行任务、推理并展现一定程度的自主性。简而言之，代理是一个具有复杂推理能力、记忆和执行任务手段的系统

![6.2 LLM-based Agent 基于大模型的智能体-v2-be260891cd2d3beb92fc0ab1bf778f96_r (1).jpg](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696164061_6.2_LLM-based_Agent_基于大模型的智能体-v2-be260891cd2d3beb92fc0ab1bf778f96_r__1_.jpg)
...
![6.2 LLM-based Agent 基于大模型的智能体-image-40.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696103717_6.2_LLM-based_Agent_基于大模型的智能体-image-40.png)
...
* **子目标分解：** agent将大任务拆分为更小的可管理的子目标，使得可以有效处理复杂任务
  ...
  * **LLM+P** 来进行一个更加长序列的整体规划。这种方法利用规划域定义语言作为中间接口来描述规划问题。整个使用过程，首先LLM将问题翻译成“问题PDDL”，接着请求经典Planner根据现有的“领域PDDL”生成一个PDDL Plan，最后将PDDL计划翻译回自然语言（LLM做的）。根本上讲，Planning Step是外包给外部工具的，当然也有一个前提：需要有特定领域的PDDL和合适的Planner
    ![6.2 LLM-based Agent 基于大模型的智能体-image-38.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696078076_6.2_LLM-based_Agent_基于大模型的智能体-image-38.png)
* **反思与完善：** agent对历史动作可以自我批评和自我反思，从错误中学习并在后续步骤里完善，从而改善最终结果的质量。Self-reflection是非常重要的一环，它允许Agent通过完善过去的行动决策和纠正以前的错误来不断改进。现实世界中的任务中，试错仍然是无法避免的，自我反思在其中发挥着至关重要的作用
  ...
  ![6.2 LLM-based Agent 基于大模型的智能体-image-39.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696080648_6.2_LLM-based_Agent_基于大模型的智能体-image-39.png)
...
![6.2 LLM-based Agent 基于大模型的智能体-image-34.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696036921_6.2_LLM-based_Agent_基于大模型的智能体-image-34.png)
...
![6.2 LLM-based Agent 基于大模型的智能体-image-35.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696046498_6.2_LLM-based_Agent_基于大模型的智能体-image-35.png)

**和Agent的对话就是：** 你有一个助手，你不是简单地直接写，先询问你是否需要进行一些网络研究，然后写下初稿，然后 **回顾** 初稿，并 **思考** 哪些部分需要修改，然后 **修改** 草稿，不断进行思考和迭代这个过程。 这个流程是一个 **思考+迭代** 的过程

![6.2 LLM-based Agent 基于大模型的智能体-image-33.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696031231_6.2_LLM-based_Agent_基于大模型的智能体-image-33.png)
...
![6.2 LLM-based Agent 基于大模型的智能体-image-37.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696064675_6.2_LLM-based_Agent_基于大模型的智能体-image-37.png)

![6.2 LLM-based Agent 基于大模型的智能体-image-36.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696056030_6.2_LLM-based_Agent_基于大模型的智能体-image-36.png)

![6.2 LLM-based Agent 基于大模型的智能体-image-32.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696025468_6.2_LLM-based_Agent_基于大模型的智能体-image-32.png)

# 6.**2.3 智能体分类**

![6.2 LLM-based Agent 基于大模型的智能体-image-28.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695983083_6.2_LLM-based_Agent_基于大模型的智能体-image-28.png)
...
![6.2 LLM-based Agent 基于大模型的智能体-image-31.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696011796_6.2_LLM-based_Agent_基于大模型的智能体-image-31.png)
...
* **CRITIC：Self-Correcting with Tool-Interactive Critiquing** 利用工具交互评价来自我修正。首先根据prompt生成回答，然后同样的LLM对答案中可能出现的错位进行criticize，最后使用相应的外部工具，如联网搜索、代码解释器对部分回答进行验证或修正。**CRITIC流程图：**
  ![6.2 LLM-based Agent 基于大模型的智能体-image-30.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696009597_6.2_LLM-based_Agent_基于大模型的智能体-image-30.png)
...
![6.2 LLM-based Agent 基于大模型的智能体-image-29.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695983762_6.2_LLM-based_Agent_基于大模型的智能体-image-29.png)

* **Tool-Integrated Reasoning Agent：** 类似PAL，但是code和推理多步交叉直到解决问题，PAL是单个code-gen step。下图是CoT、PAL和本文的对比

![6.2 LLM-based Agent 基于大模型的智能体-image-27.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695973092_6.2_LLM-based_Agent_基于大模型的智能体-image-27.png)

* **TaskWeaver：** 也是类似PAL，但是可以利用用户定义的插件
  ![6.2 LLM-based Agent 基于大模型的智能体-image-26.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695966636_6.2_LLM-based_Agent_基于大模型的智能体-image-26.png)
...
| 技术 | 背景 | 简介 | 具体方法 |
| ------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| **Reasoning and Acting**<br> | 传统CoT中存在的幻觉和错误传播问题; Act- only的方法不能充分利用LLM reasoning的高层目标规划能力<br> | 结合LLM推理和生成动作的能力，交替生成推理轨迹和特定任务的动作，重复 **产生一个thought -> 采取一个动作 -> 接收一个观测** 的流程，然后所有的信息加入prompt中，相当于保存了过去的经历的记忆。支持外部工具访问。ReAct仅需学习一到六个context examples即可泛化到新任务实例<br> | **流程及与CoT和Act-only方法的对比**![6.2 LLM-based Agent 基于大模型的智能体-image-21.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695915749_6.2_LLM-based_Agent_基于大模型的智能体-image-21.png) |
| **Reflexion** | LLM快速的从trial-and-error 中学习仍然很有挑战性，因为传统的强化学习方法需要大量的训练样本和昂贵的模型微调 | 由三个不同的模型组成：**Actor、Evaluator和Self-Reflection**。相比ReAct，多了Evaluator和Reflection。Actor来生成文本和动作，并在环境中接收观察结果。Evaluator评估Actor产生的轨迹的质量，并计算一个奖励分数以反映其性能。Self-Reflection模型则对反馈内容进行反思（做了什么，什么出错了），然后加入prompt作为memory，在任务中不断迭代优化，从而提高决策能力<br> | **流程图**![6.2 LLM-based Agent 基于大模型的智能体-image-23.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695938686_6.2_LLM-based_Agent_基于大模型的智能体-image-23.png)内部反馈是Evaluator给出的反馈，外部反馈是环境给出的标量奖励之类的反馈 |
| **Lifelong Learning Agents** | 可以在需要终身学习的真实世界任务中探索的Agents（MC等任务环境）<br> | **Voyager：** 主要包含三个部分，自动课程任务生成来探索开放世界，迭代式的生成code与环境交互执行动作，Self-Verification new skill加入skill library用于技能召回。实验中对比了ReAct、Reflexion和AutoGPT，效果碾压<br> | **流程图**![6.2 LLM-based Agent 基于大模型的智能体-image-25.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695957239_6.2_LLM-based_Agent_基于大模型的智能体-image-25.png)**实验结果**![6.2 LLM-based Agent 基于大模型的智能体-image-24.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695942288_6.2_LLM-based_Agent_基于大模型的智能体-image-24.png) |
| | | **Ghost in the Minecraft (GITM)：** 利用LLM将一开始的goal分解为subgoals，然后迭代的进行plan和执行structured text（action）。同时GITM还用了外部的知识库辅助目标分解以及存储经验<br> | **流程图**![6.2 LLM-based Agent 基于大模型的智能体-image-17.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695876279_6.2_LLM-based_Agent_基于大模型的智能体-image-17.png)**LLM Planner 示例**![6.2 LLM-based Agent 基于大模型的智能体-image-22.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695924425_6.2_LLM-based_Agent_基于大模型的智能体-image-22.png) |
...
| **技术** | **背景** | **简介** | **具体方法** |
| ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Verify-and-Edit** | CoT 通过生成可解释的推理链来提高复杂推理任务的信任和模型性能，但在知识密集型任务中仍然存在事实性问题 | 生成多条CoT，选择一些进行编辑，编辑是通过对外部信息检索然后允许LLM对这些进行增强<br> | **流程图**![6.2 LLM-based Agent 基于大模型的智能体-image-20.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695910696_6.2_LLM-based_Agent_基于大模型的智能体-image-20.png) |
| **Demonstrate-Search-Predict** | 现有工作简单将 RM 检索插入到 LM prompt中在multi-hop问题中效果不是很好 | 利用few-shot将一个问题分解为子问题，然后将对应的子问题的回答结合为最终的answer | **示例流程**![6.2 LLM-based Agent 基于大模型的智能体-image-19.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695890939_6.2_LLM-based_Agent_基于大模型的智能体-image-19.png) |
| **Iterative Retrieval Augmentation**<br> | 之前的一些检索增强LM存在两个方面的限制：一次性生成文本在long-form生成任务中容易产生幻觉采用概括性的topic作为query导致查询不到相关细节内容 | **FLARE：** 迭代的检索生成，先生成一个临时句子，然后检查句子中是否包含低可信度的token，如果有就进行检索然后重新生成句子，重复这个步骤直到完成生成 | 示例检索query生成：隐式query通过mask掉临时句子中的probs低于阈值的token生成；显式query通过要求LM生成 答案为临时句子中probs低于阈值的span 的问题，这些问题作为query去进行检索，然后将检索文档加入prompt重新生成句子<br>**流程图**![6.2 LLM-based Agent 基于大模型的智能体-image-18.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695888336_6.2_LLM-based_Agent_基于大模型的智能体-image-18.png) |
| | | **IRP：** 说明性文本生成任务。包含三个组件，Imitator、Retriever和Paraphraser。Imitator首先生成一个stylistic content plan，概述要包含在下一句中的事实，然后Retriever根据plan在语料库中检索相关facts，最后Paraphraser以plan的风格重新表述这些facts，加到输出后面作为下一个的prefix。sentence-by-sentence生成 | **流程：** Imitator是用训练集中说明性文本的expert content plan进行训练的；Retriever部分为了减轻plan中的幻觉事实entity损害模型的检索能力，文章用DistilBERT在训练集上进行了分类任务微调，即预测这个文本中的句子的index（第几个句子），因为来自不同文本的同样index的句子的事实相关entity肯定是不同的，所以这样会使得模型对于事实entity的token的attribution scores更低 训练集GT示例<br>**流程图**![6.2 LLM-based Agent 基于大模型的智能体-image-16.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695869704_6.2_LLM-based_Agent_基于大模型的智能体-image-16.png) |
...
![6.2 LLM-based Agent 基于大模型的智能体-IBrnbPEEFoxEn3xuyUScTHQUnVd.webp](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695759875_6.2_LLM-based_Agent_基于大模型的智能体-IBrnbPEEFoxEn3xuyUScTHQUnVd.webp)
...
![6.2 LLM-based Agent 基于大模型的智能体-EhxGbCr1toWeu9xA3PlcC0xpnke.webp](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695743479_6.2_LLM-based_Agent_基于大模型的智能体-EhxGbCr1toWeu9xA3PlcC0xpnke.webp)
...
![6.2 LLM-based Agent 基于大模型的智能体-XAwLbC8z7o7jvaxpc34cSnsenCg.webp](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695775781_6.2_LLM-based_Agent_基于大模型的智能体-XAwLbC8z7o7jvaxpc34cSnsenCg.webp)
...
![6.2 LLM-based Agent 基于大模型的智能体-QPAHbZJVLo2leWx5xpmc7jNnnwe.webp](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695766744_6.2_LLM-based_Agent_基于大模型的智能体-QPAHbZJVLo2leWx5xpmc7jNnnwe.webp)
...
![6.2 LLM-based Agent 基于大模型的智能体-DzTBbk7eZojlrGx0TTRc81BZncc.webp](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695728457_6.2_LLM-based_Agent_基于大模型的智能体-DzTBbk7eZojlrGx0TTRc81BZncc.webp)
...
![6.2 LLM-based Agent 基于大模型的智能体-AkOFbV5cWoXHH6xy30acnvNkngb.webp](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695723342_6.2_LLM-based_Agent_基于大模型的智能体-AkOFbV5cWoXHH6xy30acnvNkngb.webp)
...
![6.2 LLM-based Agent 基于大模型的智能体-HBZxbxh8Ho7vY8xoc2IcSkdynMf.webp](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695750694_6.2_LLM-based_Agent_基于大模型的智能体-HBZxbxh8Ho7vY8xoc2IcSkdynMf.webp)
...
![6.2 LLM-based Agent 基于大模型的智能体-image-11.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695823496_6.2_LLM-based_Agent_基于大模型的智能体-image-11.png)
...
![6.2 LLM-based Agent 基于大模型的智能体-image-15.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695861628_6.2_LLM-based_Agent_基于大模型的智能体-image-15.png)

### 单智能体应用

**BabyAGI、AutoGPT、HuggingGPT等，通过设计不同的prompt和工作流，利用OpenAI模型或者huggingface上的模型，实现任务规划、任务执行、任务排序等功能**

![6.2 LLM-based Agent 基于大模型的智能体-image-14.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695853254_6.2_LLM-based_Agent_基于大模型的智能体-image-14.png)

### 多智能体应用

**generative\_agents、MetaGPT、GPT-researcher、STORM / Co-STORM，多智能体协作交流完成任务**

![6.2 LLM-based Agent 基于大模型的智能体-image-13.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695839668_6.2_LLM-based_Agent_基于大模型的智能体-image-13.png)

![6.2 LLM-based Agent 基于大模型的智能体-image-10.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695820365_6.2_LLM-based_Agent_基于大模型的智能体-image-10.png)
...
![6.2 LLM-based Agent 基于大模型的智能体-image-12.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695831595_6.2_LLM-based_Agent_基于大模型的智能体-image-12.png)
...
![6.2 LLM-based Agent 基于大模型的智能体-image-8.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696139568_6.2_LLM-based_Agent_基于大模型的智能体-image-8.png)
...
![6.2 LLM-based Agent 基于大模型的智能体-image-9.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696148810_6.2_LLM-based_Agent_基于大模型的智能体-image-9.png)
...
![6.2 LLM-based Agent 基于大模型的智能体-image-7.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696127369_6.2_LLM-based_Agent_基于大模型的智能体-image-7.png)

![6.2 LLM-based Agent 基于大模型的智能体-image-5.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696113386_6.2_LLM-based_Agent_基于大模型的智能体-image-5.png)
...
![6.2 LLM-based Agent 基于大模型的智能体-image-6.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696122995_6.2_LLM-based_Agent_基于大模型的智能体-image-6.png)

![6.2 LLM-based Agent 基于大模型的智能体-image-3.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695992336_6.2_LLM-based_Agent_基于大模型的智能体-image-3.png)

![6.2 LLM-based Agent 基于大模型的智能体-image-4.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782696088811_6.2_LLM-based_Agent_基于大模型的智能体-image-4.png)

![6.2 LLM-based Agent 基于大模型的智能体-image-1.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695807044_6.2_LLM-based_Agent_基于大模型的智能体-image-1.png)

![6.2 LLM-based Agent 基于大模型的智能体-image-2.png](https://2479e837.cloudflare-imgbed-2q4.pages.dev/file/1782695904213_6.2_LLM-based_Agent_基于大模型的智能体-image-2.png)

## Cubox Review 增补：常见 Agent 架构模式

### 三个基础模式

| 模式 | 适合任务 | 关键风险 |
|---|---|---|
| ReAct | 搜索、问答、工具调用循环 | 轮次过多会失控，需要最大循环数和停止条件。 |
| Plan-and-Execute | 长任务、旅行计划、复杂交付 | 初始计划可能不准，需要 replanning 或人工确认。 |
| Reflection / Self-Critique | 写作、代码、报告修订 | 评审标准不清时容易自我肯定或空泛修改。 |

生产系统通常会组合使用：先规划，再用 ReAct 执行具体步骤，最后用 reflection 检查结果。

### 九种模式线索

`Agent的九种设计模式` 额外整理了 Plan-and-Solve、REWOO、LLMCompiler、Basic Reflection、Reflexion、LATS、Self-Discover、Storm 等模式。它们可以理解为对“规划、并行、反思、树搜索、资料生成”的不同组织方式。

`【Agent面试4】 ReAct还是Plan-and-Execute` 当前正文为图片，未 OCR，只作为 ReAct vs Plan-and-Execute 对比的待补充线索。

## 增补来源

- [[Agent的九种设计模式(图解+代码) - 知乎-2026-04-30]]
- [[AI Agent 几种架构模式大家好，我是双越。wangEditor 作者，前百度 滴滴 资深前端工程师，慕课网金牌讲师 - 掘金-2026-05-21]]
- [[【Agent面试4】 ReAct还是Plan-and-Execute-2026-05-20]]

## Cubox 回源复核

- 2026-06-30 已补跑 2026-05 Reviewed 回源；掘金 Agent 架构模式文通过 `#article-root` 抽到约 3073 字正文，证据升级为 `source_fetched_html`，详见 [[2025-12 to 2026-05 Cubox Source Re-Audit]]。
- [[【Agent面试4】 ReAct还是Plan-and-Execute-2026-05-20]] 已做图片 OCR，可作为面试材料/弱方法论补充；[[Agent的九种设计模式(图解+代码) - 知乎-2026-04-30]] 仍因知乎 403 只作为待核清单。
