...
> [!missing-image] 缺失图片：`images/3.5 PEFT 参数高效微调-image.png`
...
> [!missing-image] 缺失图片：`images/3.5 PEFT 参数高效微调-image-19.png`
> [!missing-image] 缺失图片：`images/3.5 PEFT 参数高效微调-image-15.png`
...
> [!missing-image] 缺失图片：`images/3.5 PEFT 参数高效微调-image-20.png`
...
> [!missing-image] 缺失图片：`images/3.5 PEFT 参数高效微调-image-14.png`
...
> [!missing-image] 缺失图片：`images/3.5 PEFT 参数高效微调-image-16.png`
...
> [!missing-image] 缺失图片：`images/3.5 PEFT 参数高效微调-image-11.png`
> [!missing-image] 缺失图片：`images/3.5 PEFT 参数高效微调-image-17.png`
...
> [!missing-image] 缺失图片：`images/3.5 PEFT 参数高效微调-image-13.png`
...
> [!missing-image] 缺失图片：`images/3.5 PEFT 参数高效微调-image-18.png`
> [!missing-image] 缺失图片：`images/3.5 PEFT 参数高效微调-image-7.png`
...
* **LoRA的实现：** LoRA旨在 **使用低秩矩阵来编码参数增量 $\Delta\phi$，在训练过程中，低秩的适应矩阵仅仅放大了对下游任务有用的特征，而不是预训练模型中的主要特征**
  ...
  * **Low Rank 低秩：** 根据内在维度的启发，LoRA认为参数更新过程中也存在一个内在秩，对于预训练权重矩阵可以用一个低秩分解来表示参数更新：
  > [!missing-image] 缺失图片：`images/3.5 PEFT 参数高效微调-image-12.png`
  ...
  * **LoRA应该作用在哪些参数矩阵？**：
    > [!missing-image] 缺失图片：`images/3.5 PEFT 参数高效微调-image-10.png`
    * 将所有微调参数都放到attention的某一个参数矩阵的效果并不好，将可微调参数平均分配到𝑊𝑞和𝑊v的效果最好
    * 即使是秩仅取4也能在Δ𝑊中获得足够的信息
  * **LoRA的最优秩r是多少？：**
    > [!missing-image] 缺失图片：`images/3.5 PEFT 参数高效微调-image-8.png`
...
> [!missing-image] 缺失图片：`images/3.5 PEFT 参数高效微调-image-4.png`
...
> [!missing-image] 缺失图片：`images/3.5 PEFT 参数高效微调-image-5.png`
...
> [!missing-image] 缺失图片：`images/3.5 PEFT 参数高效微调-image-1.png`
...
> [!missing-image] 缺失图片：`images/3.5 PEFT 参数高效微调-image-2.png`
