...
![[Extras/Excalidraw/BGP/BGP分类.excalidraw|BGP分类.excalidraw]]
...
- 说明：如果在 AS 内一台 BGP 设备收到 EBGP 邻居发送的路由后，需要通过另一台 BGP 设备将该路由传输给其他 AS，则推荐使用 IBGP。
...
    Connect --> Established: TCP Established
    Active --> OpenSent: TCP Established
    Active --> Idle: Connect Retry Timeout
    OpenSent --> OpenConfirm: Receive Correct Open
...
- Idle 状态是 BGP 初始状态。在 Idle 状态下， BGP 拒绝邻居发送的连接请求。只有在收到本设备的 Start 事件后， BGP 才开始尝试和其它 BGP 对等体进行 TCP 连接，并转至 Connect 状态。
	- 说明：Start 事件是由一个操作者配置一个 BGP 过程，或者重置一个已经存在的过程或者路由器软件重置 BGP 过程引起的。任何状态中收到 Notification 报文 或 TCP 拆链通知等 Error 事件后， BGP 都会转至 Idle 状态。
...
![[Extras/Excalidraw/BGP/IGP 引入 BGP.excalidraw|IGP 引入 BGP.excalidraw]]
...
- Local_Pref 属性表明路由器的 BGP 优先级，用于判断流量离开 AS 时的最佳路由。当 BGP 的设备通过不同的 IBGP 对等体得到目的地址相同但下一跳不同的多条路由时，将优先选择 Local_Pref 属性值较高的路由。
- Local_Pref 属性仅在 IBGP 对等体之间有效，不通告给其他 AS。Local_Pref 属性可以手动配置，如果路由没有配置 Local_Pref 属性， BGP 选路时将该路由的 Local_Pref 值按缺省值 100 来处理。
...
| 团体属性名称              | 团体属性号                    | 说明                                               |
| ------------------- | ------------------------ | ------------------------------------------------ |
| Internet            | 0 (0x00000000)           | 设备在收到具有此属性的路由后，可以将任何 BGP 对等体发送该路由                |
| No_Advertise        | 4294967042 (0xFFFFFF 02) | 设备收到具有此属性的路由后，将不向任何 BGP 对等体发送该路由                 |
| No_Export           | 4294967041 (0xFFFFFF )   | 设备收到具有此属性的路由后，将不向 AS 外对发送该路由                     |
| No_Export_Subconfed | 4294967043 (0xFFFFFF 03) | 设备收到具有此属性的路由后，将不向 AS 外发送该路由，也不向 AS 内其他子 AS 发布此路由 |
...
2. 优选本地优先级（Local_Pref）最高的路由。如果路由没有本地优先级， BGP 选路时将该路由按缺省的本地优先级 100 来处理。
...
![[Extras/Excalidraw/BGP/路由反射器.excalidraw|路由反射器.excalidraw]]
...
![[Extras/Excalidraw/BGP/备份路由反射器.excalidraw|备份路由反射器.excalidraw]]
...
![[Extras/Excalidraw/BGP/分级路由反射器.excalidraw|分级路由反射器.excalidraw]]

在实际的 RR 部署中，常用的是分级 RR 的场景。如图， ISP 为 AS100 提供 Internet 路由。AS100 内部分为两个集群，其中 Cluster1 内的四台设备是核心路由器，采用备份 RR 的形式保证可靠性。

### 同级路由反射器

![[Extras/Excalidraw/BGP/同级路由反射器.excalidraw|同级路由反射器.excalidraw]]
...
联盟示意图
![[Extras/Excalidraw/BGP/BGP 联盟.excalidraw|BGP 联盟.excalidraw]]
...
| 集群与集群之间仍然需要全连接。                        | 联盟的子 AS 之间是特殊的 GP 连接，不需要全连接。 |
| 适用于中、大规模网络                             | 适用于大规模网络。                    |
...
![[Extras/Excalidraw/BGP/路由衰减.excalidraw|路由衰减.excalidraw]]
...
![[Extras/Excalidraw/BGP/BGP 与 BFD 联动.excalidraw|BGP 与 BFD 联动.excalidraw]]
...
graph LR 
	RouterA -- " " --> RouterB 
	RouterB -- " " --> RouterC 
	classDef router fill:#1f77b4,color:#fff; 
	class RouterA,RouterB,RouterC router;
```
> 说明：如果 IBGP 邻居的建立依赖于 IGP 路由，应配置从 BGP Tracking 发现邻居不可达到 BGP 中断连接的时间间隔，使该时间间隔大于 IGP 路由收敛时间。否则，在闪断导致的 IGP 路由震荡恢复之前，可能 BGP 邻居关系就已经中断了，这样将导致不必要的 BGP 收敛。
...
![[Extras/Excalidraw/BGP/Auto FRR.excalidraw|Auto FRR.excalidraw]]
...
| 无 GR、NSR 技术的主备倒换 | 有 GR 技术的主备倒换                                                     | 有 NSR 技术的主备倒换  |
| ---------------- | ---------------------------------------------------------------- | -------------- |
| BGP 邻居重建         | BGP 邻居重建                                                         | BGP 邻居重建       |
| 路由重新计算           | 路由重新计算                                                           | 路由重新计算         |
| 转发表保持不变          | 转发表保持不变                                                          | 转发表保持不变        |
| 转发流量丢失，业务中断      | 转发流量零丢失，业务不受影响                                                   | 转发流量零丢失，业务不受影响 |
| 整网感知路由变化，路由短时震落  | 除主备倒换设备的邻居外的其他路由器感知不到路由变化                                        | 整网不感知路由变化      |
|                  | GR Restarter 需要邻居支持 GR Help 功能， GR Helper 不支持多邻居节点，同时以 GR 方式主备倒换 | 不需要邻居支持 NSR 功能 |
